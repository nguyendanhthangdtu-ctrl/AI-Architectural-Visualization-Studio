...
describe('apps/api video route (BUILD 16 Image → Video)', () => {
  ...
  it('submits a video job asynchronously (202, status running, real providerOperationName), then polling drives it to succeeded with a real downloadable output asset', async () => {
    ...
    const submitRes = await fetch(`${baseUrl}/projects/${project.id}/generations/${generationId}/videos`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    const poll1 = await fetch(`${baseUrl}/projects/${project.id}/videos/${submitBody.videoId}`);
    ...
    const poll2 = await fetch(`${baseUrl}/projects/${project.id}/videos/${submitBody.videoId}`);
    ...
    const assetRes = await fetch(`${baseUrl}${poll2Body.outputAssetUrl}`);
    ...
    const poll3 = await fetch(`${baseUrl}/projects/${project.id}/videos/${submitBody.videoId}`);
  ...
  it('marks the video failed when the provider operation errors', async () => {
    ...
    const poll = await fetch(`${baseUrl}/projects/${project.id}/videos/${submitBody.videoId}`);
  ...
  it('returns 404 for an unknown project', async () => {
    ...
    const res = await fetch(`${baseUrl}/projects/does-not-exist/generations/g1/videos`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  it('returns 404 for an unknown parent generation', async () => {
    ...
    const res = await fetch(`${baseUrl}/projects/${project.id}/generations/does-not-exist/videos`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  it('returns 404 when the source asset does not belong to the project', async () => {
    ...
    const res = await fetch(`${baseUrl}/projects/${project.id}/generations/${generationId}/videos`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  it('returns 404 for an unknown video on status poll', async () => {
    const context = createAppContext();
    await start(context);
    const { project } = await createProjectAssetAndGeneration();
    const res = await fetch(`${baseUrl}/projects/${project.id}/videos/does-not-exist`);
    expect(res.status).toBe(404);
  ...
  it('returns 503 PROVIDER_NOT_CONFIGURED for Veo when no key is set — the real, honest state right now', async () => {
    const context = createAppContext(); // no VEO_API_KEY
    await start(context);
    const { project, generationId, asset } = await createProjectAssetAndGeneration();
    const res = await fetch(`${baseUrl}/projects/${project.id}/generations/${generationId}/videos`, {
      method: 'POST',
