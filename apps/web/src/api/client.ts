 ...
 *
 * RELEASE 02: defaults to a relative (same-origin) base rather than an
 * absolute `http://localhost:8080` — the supported architecture routes
 * `/projects`, `/assets`, `/auth`, `/health`, `/metrics` to apps/api via a
 * same-origin proxy (`vite.config.ts`'s dev server, a shared reverse-proxy
 * domain in production), which is *required* for the session cookie to work
 * at all under `SameSite=Strict` (see apps/api/src/auth/session.ts's own
 * comment on why `Strict` is safe here). `VITE_API_BASE_URL` still overrides
 * this for a genuinely cross-origin deployment.
 */
...
/**
 * `credentials: 'include'` on every request — required for the browser to
 * both send an existing session cookie and store a new one `Set-Cookie`
 * response returns (same-origin requests technically default to including
 * cookies already, but being explicit here means this keeps working even if
 * a caller points `VITE_API_BASE_URL` at a genuinely different origin that's
 * on the server's CORS allowlist).
 */
...
/** RELEASE 02 — only succeeds when the server has `REGISTRATION_SECRET` configured and the supplied value matches it. */
export async function register(params: { email: string; password: string; registrationSecret: string }): Promise<AuthenticatedUser> {
...
export async function login(params: { email: string; password: string }): Promise<AuthenticatedUser> {
...
/** Returns `null` (not a thrown error) for the common "not signed in yet" case — callers use this to decide whether to show the sign-in gate. */
...
export async function uploadAsset(projectId: string, file: File): Promise<UploadedAsset> {
  const res = await apiFetch(`/projects/${projectId}/assets`, {
    method: 'POST',
...
export async function runAnalysis(projectId: string, assetId: string): Promise<AnalysisResult> {
  const res = await apiFetch(`/projects/${projectId}/analysis`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
...
export async function extractReferenceVisualLanguage(
  ...
  const res = await apiFetch(`/projects/${projectId}/references`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
...
export async function runGeneration(projectId: string, params: RunGenerationParams): Promise<GenerationResult> {
  const res = await apiFetch(`/projects/${projectId}/generations`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(params),
  });
...
export async function runEdit(projectId: string, generationId: string, params: RunEditParams): Promise<EditResult> {
  const res = await apiFetch(`/projects/${projectId}/generations/${generationId}/edits`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(params),
  });
...
export async function runView(projectId: string, params: RunViewParams): Promise<ViewResult> {
  const res = await apiFetch(`/projects/${projectId}/views`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(params),
  });
...
export async function runVideo(projectId: string, generationId: string, params: RunVideoParams): Promise<RunVideoResult> {
  const res = await apiFetch(`/projects/${projectId}/generations/${generationId}/videos`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(params),
  });
...
export async function getVideoStatus(projectId: string, videoId: string): Promise<VideoStatusResult> {
  const res = await apiFetch(`/projects/${projectId}/videos/${videoId}`);
  if (!res.ok) throw new ApiError(await parseErrorEnvelope(res));
...
export async function runQc(projectId: string, generationId: string, params: RunQcParams): Promise<RunQcResult> {
  const res = await apiFetch(`/projects/${projectId}/generations/${generationId}/qc`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(params),
  });
...
export async function regenerate(projectId: string, generationId: string, params: RunRegenerateParams): Promise<GenerationResult> {
  const res = await apiFetch(`/projects/${projectId}/generations/${generationId}/regenerate`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(params),
  });
